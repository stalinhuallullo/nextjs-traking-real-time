...
export default function Home() {
    ...
    useEffect(() => {
        console.log("localStorageRute map ==> ", localStorageRute)
    ...
    const saveAndShowPoints = (drawInstance: MapboxDraw, mapInstance: mapboxgl.Map) => {
        const savedPoints = localStorage.getItem('_WHEREABOUTS_');
        ...
        if (savedPoints) {
            ...
            (async () => {
                const data: RoutesWhereabout = await getData();
                console.log("data ===> ", data)

                drawInstance.add(data.whereabout);
                drawRoute(data.whereabout, mapbox.map);
                setRouteExists(true);
                mapInstance.removeControl(drawInstance);  // Remove drawing control
            })()
        }
